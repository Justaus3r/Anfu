...
if CheckOs() == "Windows": 
	#if os is windows then we won't show selected option with stylish text(cuz of problem of cmd showing unicode characters) ,instead we will use >> to indicate selected os
	_menu_list_1 = ["Encrypt", "Decrypt", "About", "Exit"]
	_menu_list_2 = [">>Encrypt", ">>Decrypt", ">>About", ">>Exit"]
	...
	#if os  is linux then we will use stylish font
	_menu_list_1 = ["Encrypt", "Decrypt", "About", "Exit"]
...
def _get_filename(stdscr):
	filename = Tk()
	if CheckOs() == "Windows":
		filename.FileName = filedialog.askopenfilename(initialdir="C:\\", title="Select a File",filetypes=(("All Files", "*.*"),))
		filename.destroy()
		return filename.FileName
	else:
		#if not windows then linux,IGNOREEEEEEEEE MAC
		filename.FileName = filedialog.askopenfilename(initialdir="/home", title="Select a File",filetypes=(("All Files", "*.*"),))
		filename.destroy()
		return filename.FileName
...
def _get_keyfile(stdscr):
	keyfile = Tk()
	if CheckOs() == "Windows":
		keyfile.KeyfileName = filedialog.askopenfilename(initialdir="C:\\", title="Selecta Key File",filetypes=(("Anfu Keyfiles", "*.anky"),))
		keyfile.destroy()
		...
		# if user press enter key then check the _current_row_index and show subMenus According to it
			...
			if _current_row_index == 0:
				...
				while True:
					...
					stdscr.clear()
					if _key_enc_menu == curses.KEY_UP and _current_row_index_submenu_enc > 0:
						_current_row_index_submenu_enc -= 1
					elif _key_enc_menu == curses.KEY_DOWN and _current_row_index_submenu_enc < len(
							_sub_menu_list_encrypt) - 1:
						_current_row_index_submenu_enc += 1
						...
						if _current_row_index_submenu_enc == 0:
							...
							text = "Press any key to Select a File,Esc to return to Menu"
							stdscr.attron(curses.color_pair(69))
							Display_text_in_center(stdscr, text)
							stdscr.attroff(curses.color_pair(69))
				...
				while True:
					...
					stdscr.clear()
					if _key_dec_menu == curses.KEY_UP and _current_row_index_submenu_dec > 0:
						_current_row_index_submenu_dec -= 1
					elif _key_dec_menu == curses.KEY_DOWN and _current_row_index_submenu_dec < len(
							_sub_menu_list_decrypt):
						_current_row_index_submenu_dec += 1
				...
				Label(_about_window,text="An open source program for Encryping Files and Folders").pack()
				Label(_about_window,text="Version:0.1").pack()
				Label(_about_window,text="Distributed under GPLV3").pack()
				Label(_about_window,text="Star the repo(github.com/Justaus3r/Anfu),if you think it deserves.").pack()
